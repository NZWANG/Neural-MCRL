...
class EEGDataset():
    ...
    def __init__(self, data_path, exclude_subject=None, subjects=None, train=True, time_window=[0, 1.0], classes = None, pictures = None, val_size=None):
        ...
        if self.classes is None and self.pictures is None:
            ...
            text_features_filename = os.path.join(f'{model_type}_text_features_train.pt') if self.train else os.path.join(f'{model_type}_text_features_test.pt') # 文本特征文件名
            img_features_filename = os.path.join(f'{model_type}_img_features_train.pt') if self.train else os.path.join(f'{model_type}_img_features_test.pt') # 图像特征文件名  
    ...
    def load_data(self):
        ...
        if self.train:
            text_file_path = os.path.join('/root/autodl-tmp/EEG2Vision/Retrieval/texts/this_is_a_description_texts', 'img_to_text_training.pkl')
        else:
            text_file_path = os.path.join('/root/autodl-tmp/EEG2Vision/Retrieval/texts/this_is_a_description_texts', 'img_to_text_test.pkl')
